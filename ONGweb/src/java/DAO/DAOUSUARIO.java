...
//select nom_usu, ape_pat + ' ' + ape_mat as apellidos,CAST(DAY(fecha_nac) as varchar) + '-' + CAST(MONTH(fecha_nac) as varchar) + '-' + CAST(YEAR(fecha_nac) as varchar) as fecha_nac,dir_usu,tel_usu,usuario,foto_usu from Usuario
      ...
      public static int actualizaDatosUsuario(DTOUSUARIO dtousuario){
          int res = 0;
          try {
              CallableStatement cst = Conexion.getConexion().prepareCall("{call actualizaDatosUsuario(?,?,?,?,?,?,?,?)}");
              cst.setInt(1,dtousuario.getCodigo());
              cst.setString(2,dtousuario.getNombre());
              cst.setString(3,dtousuario.getApellido_pat());
              cst.setString(4,dtousuario.getApellido_mat());
              cst.setString(5,dtousuario.getFechaNacimiento());
              cst.setString(6,dtousuario.getDireccion());
              cst.setString(7,dtousuario.getTelefono());
              cst.setString(8,dtousuario.getFoto());
              res = cst.executeUpdate();
          } catch (SQLException e) {
              System.err.println(e.toString());
          }
