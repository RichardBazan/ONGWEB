...
public class DAOCASAREFUGIO {
    ...
    public List<DTOCASAREFUGIO> readAllCasaRefugio() throws SQLException {
         ...
         try {
            cst = Conexion.getConexion().prepareCall("select cod_casa,nom_casa,dir_casa,tel_cont,CAST(DAY(cr.fecha_reg) as varchar) + '/' + CAST(MONTH(cr.fecha_reg) as varchar) + '/' + CAST(YEAR(cr.fecha_reg) as varchar) as fecha_reg,estado_casa ,u.usuario from CasaRefugio cr inner join Usuario u on u.cod_usu = cr.cod_usu");
